...
export class BartrPointSettingController {
    ...
    //for new create
@Post()
async createBartrPointValue( @Body() createBartrPointDTO: BartrPointValueDTO): Promise<IResponse> {
   console.log('createBartrSignupReward')
    try {
        const response = await this._bartrSettingService.createBartrPoint(createBartrPointDTO);
