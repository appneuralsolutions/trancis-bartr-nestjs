...
//for new create
@Post()
async createBartrSignupReward( @Body() createBartrSignupRewardDTO: BartrSignupRewardDTO): Promise<IResponse> {
   console.log('createBartrSignupReward')
    try {
        const response = await this._bartrSettingService.createBartrSignupReward(createBartrSignupRewardDTO);
        console.log(response,'bartr reward point created')
        const responseObj: any = {statusCode:201,data:response,success:true}
        return responseObj;
    } catch (error) {
        console.log('error',error)
        const responseObj: any = {statusCode:500,data:[],errorMessage:error,success:false}
        return responseObj;
    }
}
...
async getBartrSignupReward(){
    try {
        const response = await this._bartrSettingService.getBartrSignupRewardValue();
        console.log(response,'get bartr reward points')
        const responseObj: any = {statusCode:200,data:response,success:true}
        return responseObj;
    } catch (error) {
        console.log(error)
        const responseObj: any = {statusCode:500,data:[],errorMessage:error,success:false}
